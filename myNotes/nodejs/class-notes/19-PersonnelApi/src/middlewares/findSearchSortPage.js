...
// Searching & Sorting & Pagination:  
    ...
    res.getModelList = async function (Model, populate = null) {
        ...
        return await Model.find(search).sort(sort).skip(skip).limit(limit).populate(populate).collation({ locale: 'en', strength: 2 })
    }

    // Details:
    res.getModelListDetails = async function (Model) {
        const data = await Model.find(search).collation({ locale: 'en', strength: 2 })
        let details = {
            search,
            sort,
            skip,
